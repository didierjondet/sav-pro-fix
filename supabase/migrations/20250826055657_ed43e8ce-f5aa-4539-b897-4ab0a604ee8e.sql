-- Ajouter une colonne pour traquer les quantités réservées
ALTER TABLE public.parts 
ADD COLUMN reserved_quantity integer DEFAULT 0;

-- Fonction pour gérer les réservations de stock lors de l'ajout de pièces SAV
...
-- Fonction pour gérer la libération des stocks lors de la finalisation/annulation des SAV
...
-- Créer les triggers
CREATE TRIGGER sav_part_stock_reservation
  AFTER INSERT OR UPDATE OR DELETE ON public.sav_parts
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_sav_part_stock_reservation();

CREATE TRIGGER sav_completion_stock
  AFTER UPDATE OR DELETE ON public.sav_cases
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_sav_completion_stock();

-- Fonction utilitaire pour obtenir le stock disponible
...
-- Mettre à jour les réservations existantes pour les SAV en cours
UPDATE public.parts 
SET reserved_quantity = COALESCE((
  ...
  AND sc.status NOT IN ('delivered', 'cancelled')
), 0);