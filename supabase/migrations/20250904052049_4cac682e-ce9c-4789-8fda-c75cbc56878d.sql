...
DO $$
BEGIN
  -- Vérifier et ajouter monthly_sms_used si elle n'existe pas
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shops' AND column_name = 'monthly_sms_used') THEN
    ALTER TABLE public.shops ADD COLUMN monthly_sms_used integer DEFAULT 0;
  END IF;
  
  -- Vérifier et ajouter purchased_sms_credits si elle n'existe pas
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shops' AND column_name = 'purchased_sms_credits') THEN
    ALTER TABLE public.shops ADD COLUMN purchased_sms_credits integer DEFAULT 0;
  END IF;
  
  -- Vérifier et ajouter last_monthly_reset si elle n'existe pas
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shops' AND column_name = 'last_monthly_reset') THEN
    ALTER TABLE public.shops ADD COLUMN last_monthly_reset date DEFAULT CURRENT_DATE;
  END IF;
END $$;
