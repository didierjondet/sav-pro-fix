...
import { Plus, MessageSquare, Send, AlertCircle, Clock, CheckCircle, XCircle, Search } from 'lucide-react';
...
export default function Support() {
  ...
  const {
    ...
    sendMessage,
    markAsRead
  } = useSupportMessages(selectedTicket?.id);
  const {
    toast
  } = useToast();
  const filteredTickets = tickets.filter(ticket => ticket.subject.toLowerCase().includes(searchTerm.toLowerCase()) || ticket.description.toLowerCase().includes(searchTerm.toLowerCase()));
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input id="subject" placeholder="Décrivez brièvement votre problème" value={newTicketSubject} onChange={e => setNewTicketSubject(e.target.value)} />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <Textarea id="description" placeholder="Décrivez votre problème en détail..." value={newTicketDescription} onChange={e => setNewTicketDescription(e.target.value)} rows={4} />
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-200px)]">
                ...
                <Card className="flex flex-col">
                  <CardHeader className="pb-3">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Rechercher un ticket..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
                    </div>
                  </CardHeader>
                  <CardContent className="flex-1 min-h-0">
                    <ScrollArea className="h-full">
                      <div className="space-y-3">
                        ...
                        return <Card key={ticket.id} className={`cursor-pointer transition-all hover:shadow-md ${selectedTicket?.id === ticket.id ? 'ring-2 ring-primary' : ''}`} onClick={() => handleSelectTicket(ticket)}>
                ...
                <Card className="flex flex-col">
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2">
                      <MessageSquare className="h-5 w-5" />
                      {selectedTicket ? <div className="flex-1">
                  ...
                  <CardContent className="flex-1 flex flex-col min-h-0">
                    {selectedTicket ? <>
                        ...
                        <ScrollArea className="flex-1 mb-4 h-96">
                          <div className="space-y-3 p-2">
                            {messages.length === 0 ? <div className="text-center text-muted-foreground py-8">
                              ...
                              </div> : messages.map(message => <div key={message.id} className={`flex ${message.sender_type === 'shop' ? 'justify-end' : 'justify-start'}`}>
                                  <div className={`max-w-[80%] p-3 rounded-lg ${message.sender_type === 'shop' ? 'bg-blue-500 text-white rounded-br-sm ml-auto' : 'bg-green-500 text-white rounded-bl-sm mr-auto'}`}>
                        ...
                        {/* Zone de saisie */}
                        <div className="flex gap-2">
                          <Textarea placeholder="Tapez votre message..." value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={handleKeyPress} className="flex-1 min-h-[80px] resize-none" />
                          <Button onClick={handleSendMessage} disabled={!newMessage.trim()} size="sm" className="self-end">
